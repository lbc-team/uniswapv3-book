...
def publish_article(filename, force=False):
    ...
    # 26111 Chris Dixon
    # 35019 howtodefi
    # decert.me 用户 id 13917

    user_id = 15  
    category_id = 27  # 27: DeFi  7 # 4: 通识   # 3: 比特币  以太坊": 5
    ...
    match = re.search(r'chapter_(\d+)', file_basename)

    if match:
        chapter_number = int(match.group(1)) / 2
        if (chapter_number >= 10):
            chapter_number = 9
        base_date = datetime.strptime("2026-01-01", "%Y-%m-%d")
        new_date = base_date + timedelta(days=chapter_number)
        createday = new_date.strftime("%Y-%m-%d")
        print(f"从文件名 {file_basename} 提取数字 {chapter_number}，计算日期为: {createday}")
    else:
        createday = "2026-01-01"
        print(f"未能从文件名 {file_basename} 中提取数字，使用默认日期: {createday}")
